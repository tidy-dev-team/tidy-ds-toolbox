...
/** One entry in a group: the component name, its key, and an optional node type. */
export type ComponentEntry = [name: string, key: string, type?: string];

/**
 * A named category of components.
 *
 * The name lives on the group, not in a parallel array, so a heading can never
 * be separated from the items it names by filtering (#205).
 */
...
export const componentGroups: ComponentGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      [
        ...
        "7f58a9fa0837ad52cf63ae52b08ed7de88c359bd",
      ],
    ],
  },
  {
    name: "Link",
    items: [["Link", "7c987b097108e85d6b25ab2037f58013b41648da"]],
  },
  {
    name: "Slider",
    items: [
  ...
  },
  {
    ...
    items: [
      [
        ...
        "d1cbee420e90c775a15a3eee0e9e61bd32db4850",
      ],
      [
        "Search / Simple / Contained",
        "76fee6a0f51aa738a7463096bc84059c1ea9dd1b",
      ],
      [
  ...
  {
    name: "Dropdown",
    items: [["Dropdown", "048ccd15014ac0b119839f23aadee79469b32565"]],
  },
  {
    name: "List",
    items: [
  ...
  {
    name: "Progress Bar",
    items: [["Progress Bar", "7435404a33616ed08206d7c68e112e7c080158c7"]],
  },
  {
    name: "Snackbar",
    items: [
  ...
  {
    name: "Modal",
    items: [["Modal", "b15987de0b80d9a9961b1a581e82ab23ac991c8b"]],
  },
  {
...
componentGroups.forEach((group) => {
  group.items.forEach(([name, key, type = "component"]) => {
...
/**
 * The groups to show for a search term, each still carrying its own name.
 *
 * A group with no matching item is dropped. Because the name travels with the
 * items, dropping a group cannot shift a heading onto another group's items.
 */
export function visibleGroups(searchTerm: string): ComponentGroup[] {
  ...
  return componentGroups
    .map((group) => ({
      name: group.name,
      items: term
        ? group.items.filter(([name]) => name.toLowerCase().includes(term))
        : group.items,
    }))
