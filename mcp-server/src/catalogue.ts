...
export const CATALOGUE: CatalogueEntry[] = [
  ...
  },
  {
    ...
    module: "utilities",
    summary:
      "Write the two searchability lines on each component's description: an 'Also known as:' line listing the component's alternative names, and a Hebrew-scrambled 'misprint' line. Idempotent: an existing alias line is merged with, an existing misprint line replaced. Components whose name is absent from the alias table come back in `withoutAliases` and get no alias line. Fails atomically if any nodeId is missing or not a component. Stoppable while looking the ids up - a stop then writes nothing, and re-running is safe. Once writing begins a stop is refused and the run completes, so the atomic promise holds either way.",
    inputSchema: {
      nodeIds: z
        .array(z.string())
        .min(1)
  ...
  },
  {
    ...
    module: "component-labels",
    summary:
      "Build variant labels around a component set's top and left edges. Pass nodeId, or omit it to use the current selection. The `labels` object maps each axis (top, left, secondTop, secondLeft) to a variant property name on the set; empty string means no label on that axis. Errors if any axis references an unknown variant property. Stoppable between build steps (per label axis, then grouping, then extraction): a stopped run reports which steps finished; the labels it drew stay on the canvas and were not removed. NOT idempotent - re-running draws a second set of labels beside the stopped run's rather than completing it, so delete the drawn labels first if a stop happens.",
    inputSchema: {
      nodeId: z
        .string()
        .optional()
  ...
  },
  {
    ...
    module: "ds-explorer",
    summary:
      "Place a registered DS Explorer component SET onto a page as an editable clone, ready to be labelled by tidy_component_labels_build. By default (localize='full') the clone is de-linked from Kido-DS: nested instances are detached into frames and paint/text/effect styles are localized; variables/tokens stay bound to Kido-DS. Defaults to the current page and the viewport centre. Returns the new nodeId so it can be piped into tidy_component_labels_build. Errors WRONG_NODE_TYPE if the named component is a single component (not a set). Stoppable after the import but before anything is placed - a stop then leaves the canvas unchanged and re-running is safe. Once the clone exists the run is not interrupted: a half-localized clone is the silent-relinking hazard this Operation exists to prevent.",
    inputSchema: {
      name: z
        .string()
        .describe(
