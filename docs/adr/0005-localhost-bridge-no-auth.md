...
The MCP server binds the Bridge websocket to `localhost:9876` and accepts any connection that arrives. No token, no trust-on-first-use, no rate limiting. The plugin's `manifest.json` whitelists exactly `ws://localhost:9876` under `networkAccess.devAllowedDomains`; production `allowedDomains` holds the usage-analytics ingest origin and never the Bridge socket, so the agent surface is dev-only. Both endpoints use the `localhost` hostname rather than `127.0.0.1` because Figma's manifest validator rejects IP literals as "not a valid URL"; binding the server to the same name avoids IPv4/IPv6 resolution drift.
...
Nothing in this class is exposed today, and no Operation should enter it without revisiting this ADR first.

The claim above is checkable against the registered Operations, which is the point of stating it this way.
...
- `tidy_misprint_apply` is the closest case to the line and the reason this section names a line at all. See below.

Of the seven Query Operations, six write nothing at all.
...
Two details belong here rather than there.
...
We accept this.
The replacement is confined to the single matching line, the rest of a multi-line description is preserved, the content it overwrites is machine-generated scramble in every real case, and the alternative of never reapplying leaves wrong search text in place.
It is recorded here so the next reader does not have to rediscover it, and so a future Operation that wants the same latitude has to argue for it.

## Where input validation sits

The Zod schemas in the MCP server's catalogue are **documentation, not enforcement**.
Their job is to tell an agent how to call an Operation, and they do it well.
They sit in a separate process on the far side of the socket this ADR leaves unauthenticated, so they were never a boundary and must not be read as one.
