...
- **Constraints are asymmetric.** Maximums (length, item count) are hard and universal; minimums are presence-only — slots are required or optional, and a present list slot needs only ≥1 item, never a forced count. Enforcing a minimum count would induce invented filler, which contradicts the never-invent principle this ADR and ADR-0008 exist to uphold. A Section with neither derived nor worth-authoring content is skipped, not padded.
