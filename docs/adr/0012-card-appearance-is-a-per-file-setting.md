...
- **A publish sweeps its own output across the whole file before drawing.** Removing only the cards about to be redrawn leaves the ones that should no longer exist: delete the last note about a Subject and it drops out of the notes, so nothing rebuilds its card and nothing removes it either. Same for a Subject whose node has since been deleted.
  The sweep is by stamp, plus the one legacy aggregate name, plus `<Subject>-release-notes` for a Subject that still has notes. Deliberately **not** by the bare `-release-notes` suffix, which `isOwnedCard` matches and Clear Canvas sweeps with: a designer frame called `client-release-notes` carries no stamp, and claiming it automatically on every publish would destroy their work without asking. The two rules differ by how much damage a wrong answer does, `isOwnedCard` stays a superset so Clear Canvas can never strand what a publish replaces, and the asymmetry is fixture-tested.
