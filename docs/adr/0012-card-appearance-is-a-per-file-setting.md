...
- **A publish sweeps every card this module owns before drawing.** Removing only the cards about to be redrawn leaves the ones that should no longer exist: delete the last note about a Subject and it drops out of the notes, so nothing rebuilds its card and nothing removes it either. Same for a Subject whose node has since been deleted. A publish therefore uses the same ownership rule as Clear Canvas. This collapses three sweeps into one, and `isAggregateCard` and `isCardForSubject` are gone with it: three rules that had to agree became one that cannot disagree with itself.
