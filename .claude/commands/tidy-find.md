...
allowed-tools: ["mcp__tidy-ds-toolbox__tidy_misprint_find_components"]
---

Call `mcp__tidy-ds-toolbox__tidy_misprint_find_components` to find components in the active Figma file.
...
- If the user wrote a free-form glob without `pattern=`, treat it as the namePattern (e.g. `/tidy-find file Btn*`).
...
- If the result is non-empty, remind the user the ids can be passed to `/tidy-misprint` to apply the misprint line.
