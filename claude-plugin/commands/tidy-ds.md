...
allowed-tools:
  ...
  - "mcp__plugin_tidy-ds_tidy-ds-toolbox__tidy_ds_explorer_list_components"
  - "mcp__tidy-ds-toolbox__tidy_ds_explorer_get_component"
  - "mcp__plugin_tidy-ds_tidy-ds-toolbox__tidy_ds_explorer_get_component"
  - "mcp__tidy-ds-toolbox__tidy_ds_explorer_place_set"
  - "mcp__plugin_tidy-ds_tidy-ds-toolbox__tidy_ds_explorer_place_set"
---
...
- If `$ARGUMENTS` is empty, call `tidy_ds_explorer_list_components` with `{}` and report the count + first 20 names. Suggest the user pass a name next.
...
- Otherwise the first token is the exact component name. Then look at the remaining tokens:
  - `--place` → call `tidy_ds_explorer_place_set` with `{ name }`. Optional `x=<num>` / `y=<num>` / `pageId=<id>` / `localize=<none|detach|styles|full>` tokens become extra params. By default the placed clone is de-linked from Kido-DS (`localize='full'`: nested instances detached into frames + paint/text/effect styles localized); pass `localize=none` to keep the old fully-linked behavior, or `detach`/`styles` for one half only. Variables/tokens always stay bound to Kido-DS. Report the returned `nodeId` and remind the user it can be piped into `/tidy-ds:tidy-labels <nodeId>`.
...
- Remind the user: pipe `nodeId` into `/tidy-ds:tidy-labels <nodeId>` to build variant labels on it.
...
- On `WRONG_NODE_TYPE` from `place_set`, the named component is a single component (not a set). Suggest using `/tidy-ds:tidy-ds <name>` (without `--place`) to inspect it instead.
