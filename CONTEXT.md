...
**Documentation Page**:
The output artifact: a single canvas layout documenting one component (set), built as a left-to-right series of **Sections** — Variants, Component Breakdown, Mode, Usage Guidelines, Related Components. This **horizontal** arrangement is the only one: the panel offered a vertical alternative until [ADR-0010](docs/adr/0010-one-documentation-page-layout.md) removed the choice, and its rendering code is parked behind a constant rather than deleted. Layout was never a Doc Spec field and still is not — it is a **code-owned rendering branch**, per [ADR-0006](docs/adr/0006-code-as-template-for-doc-layout.md). The generator introduces **no linkage to Kido's shared libraries or other clients' files** — the only external reference it ever creates is to the source component, which lives in the *current* project (acceptable).
...
**Release Note**:
One authored statement that something in the design system changed, belonging to exactly one **Sprint** and describing exactly one **Subject**, stamped with a **Tag**, an author and a creation date. Notes are the only authored data in the module; everything else (cards, aggregate page, CSV) is a rendering of them.
...
- **A Documentation Page is a fixed, ordered set of Section types.** That set is Variants, Component Breakdown, Mode, Usage Guidelines, Related Components, and it is never reordered or invented per component. Each Section is emitted only when the component has applicable content (**skip-when-empty**: a flat component skips Variants, a single-theme component skips Mode). The parked vertical layout has its own set — Header, Component Variants matrix, Constraints, Dos and Don'ts — which no build reaches. See [ADR-0006](docs/adr/0006-code-as-template-for-doc-layout.md) and [ADR-0010](docs/adr/0010-one-documentation-page-layout.md).
