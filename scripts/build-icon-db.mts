...
const mdiSource: LibrarySource = {
  ...
  extractIcons: async () => {
    ...
    const meta = JSON.parse(
      readSvgFile(
        path.join(repoRoot, "node_modules", "@mdi", "svg", "meta.json"),
      ),
...
function renderAndHash(
  ...
  const samples = rgbaToLetterboxGrayscale(pixels, width, height, 32);
...
async function main(): Promise<void> {
  ...
  const outputDir = path.join(repoRoot, "src", "plugins", "iconfinder", "db");
  ...
  const gzipB64 = Buffer.from(gzipSync(strToU8(json), { level: 9 })).toString(
    "base64",
  );
