...
- `src/shared/operations/registry.ts` — plugin-main-thread `registerOperation` / `dispatch` / `bindSession`.
  ...
  Nothing today declares `cancellable`, so `not_cancellable` is the honest answer for every Operation in the plugin, and that is the deliverable rather than a gap: cancellation in this sandbox is cooperative and always will be, because Figma offers no way to interrupt a running handler.
  Adopting it is one flag on the spec plus a `ctx.cancellation` check and a `yieldToMain()` in the loop - without the yield the incoming message never gets a turn on the queue and the flag can never be seen set.
