...
- `mcp-server/` — Node MCP server that **listens** on `127.0.0.1:9876` and accepts the plugin's outbound connection (plugin sandbox can't accept inbound, hence inverted server/client roles).
  ...
  Nothing is cancelled at a timeout, so an `execute` is told the work continues, that the file may still be being written, and to check the canvas before calling again - never to retry, because a second `tidy_ds_template_run` over the first stamps the pages twice.
