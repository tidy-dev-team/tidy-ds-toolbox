...
Add or drop a check by editing that one table: the run registry, the default order, the agent-facing `CHECK_IDS` list, and the row count are all derived from it, and `checklist-catalogue.test.ts` asserts the join rather than restating today's headcount.
